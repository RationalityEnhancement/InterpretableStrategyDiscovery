...
def is_max_in_branch(state, action):
    ...
    predec = is_predecessor_max_val(state, action)
    max_val = state.get_max_dist_value()
    node = state.node_map[action]
...
def is_predecessor_max_val(state, action):
    if action == 0:
        return False
    node = state.node_map[action]
    max_val = state.get_max_dist_value()
...
def is_previous_observed_max_leaf(state, action):
    ...
    return was_leaf and state.is_previous_max()#was_on_best_path and state.is_previous_max()
    ...
    grammar = {
        START : (#[['any_(st, act, ', PRED, ', ', PRED, ')'],
                  #[['all_(st, act, lambda st, act: ', LIST_PRED_AMONG_DEPTH, ')'],
                   ...
                   ['is_previous_observed_leaf(st, act)'],

...
def is_observed(state, action, node_labels=None):
    """ 
    ...
    if node_labels is not None:
        return action in node_labels and node.observed
...
def one_best_subtree(state, action):
    roots = [node for node in state.node_map.values() if node.is_root()]
    paths = state.get_path_expected_values()
    best_paths = [k for k,v in paths.items() if v == max(paths.values())]
    prev_root_counter = 0
    for root in roots:
        root_counter = 0
        for best_path in best_paths:
            nodes_best_path = state.branch_map[best_path]
            if root.label in nodes_best_path:
                root_counter += 1
        if prev_root_counter != 0 and root_counter != 0:
            return False
        elif prev_root_counter != 0 and root_counter == 0:
            pass
        else:
            prev_root_counter = root_counter
    return True
...
def has_parent_highest_value(state, action, node_labels):
    """ 
    This node' has a parent with an uncovered value/expected value
    that is higher than any other observed parent's value for the nodes 
    from 'node_labels'
    """
    if action == 0 or action not in node_labels:
        return False
    node = state.node_map[action]
    node_parent_val = node.parent.value if node.parent.observed else 0
    for nl in node_labels:
        parent_nl = state.node_map[nl].parent
        if not parent_nl:
            continue
        parent_nl_val = parent_nl.value if parent_nl.observed else 0
        if parent_nl_val > node_parent_val:
            return False
    return True
...
def has_most_branches(state, action, node_labels):
    ...
    num_branches_node = len(branches_node)
    for nl in node_labels:
        branches_nl = state.reverse_branch_map[nl]
        if len(branches_nl) > num_branches_node:
            return False
    return True
...
def create_grammar():
    grammar = {
        ...
        GENERAL_PRED : ([['is_none_observed(st, act)'],
                         ...
                         ['is_observed_in_branch(st, act)'],
                         ['one_best_subtree(st, act)'],
                         ...
                         ['termination_return(st, act, ', NUMNUM, ')']],
                         [1./24.] * 24),

        NUM : ([[str(i)] for i in range(9)],
                [1./9.] * 9),

        NUMNUM : ([[str(i)] for i in range(25)],
                   [1./25.] * 25),

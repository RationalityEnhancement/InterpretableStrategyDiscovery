...
jsPsych.plugins['mouselab-mdp'] = (function() {
  var Arrow, Edge, KEYS, LOG_DEBUG, LOG_INFO, MouselabMDP, NULL, PRINT, RIGHT_MESSAGE, SIZE, State, Note, TOP_ADJUST, LEFT_ADJUST, TRIAL_INDEX, Text, angle, checkObj, dist, plugin, polarMove, redGreen, removePrivate, round, Flowchart, Tutor, Instructions, DivTutor;
  PRINT = function(...args) {
  ...
  MouselabMDP = class MouselabMDP {
    constructor(config) {
      var prs, blockName, centerMessage, blockTitle, lowerMessage, prompt, rightMessage, size, trial_id;
      ...
      if (this.energyLimit) {
        ...
        if (this._block.energyLeft == null) {
          ...
          blockTitle += `Round: ${this._block.trialCount + 1}/${this._block.timeline.length}`;
      }
    ...
    async clickState(g, s) {
      ...
      if (this.stateLabels && this.stateDisplay === 'click' && !g.revealed) {
        // provoke tutor feedback
        if(this.tutor && this.tutor.show_feedback){
          this.tutor.checkState(s) ? this.displayFeedback('Yes, this node was on a best path.', true, 2) : this.displayFeedback('No, this node was not on a best path.',false ,2);
        }
        ...
        if(this.highlight_best_path) this.highlightBestPath();
    ...
    highlightBestPath(){
      ...
      let value = 0;
      let best_path_nodes = [];
      let paths = {"3": [1,2,3], "4": [1,2,4], "7": [5,6,7], "8": [5,6,8], "11": [9,10,11], "12": [9,10,12]}

      // find max and append nodes of that path
      for(let n in this.notes){
        value = parseInt(this.notes[n].label.text);
        if(value > max){
          max = value;
          best_path_nodes = paths[parseInt(n)];
        } else if (value == max) {
          best_path_nodes = best_path_nodes.concat(paths[parseInt(n)]);
        }
      }
      best_path_nodes = [...new Set(best_path_nodes)];

      //highlight nodes
      this.unhighlightAllNodes();
      for(let n of best_path_nodes){
        this.states[n].highlight('lightgreen');
      }
      console.log(best_path_nodes);
    }
  ...
  DivTutor = class DivTutor extends Tutor{
    constructor(container, config = {}){
      ...
      $('<div>', {
        ...
        html: "Reminder: The best path is marked green.",
      }).appendTo(this.container);
  ...
  State = class State {
    ...
    highlight(color) {
      let colors = {'orange': '#FFD700', 'red': '#d26e6e', 'lightgreen': 'rgba(69, 140, 69, 0.25)'}

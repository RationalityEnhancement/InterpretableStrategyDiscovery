  ...
  MouselabMDP = class MouselabMDP {
    ...
    createFlowchart(flowchart){
      if(!jQuery.isEmptyObject(flowchart)){
        ...
        if(!flowchart.force_correct_clicking) flowchart.force_correct_clicking = false;
        if(!flowchart.force_click_all_correct_nodes) flowchart.force_click_all_correct_nodes = false;
        for(var i in this.states){
          if(i != 0){
            let s = parseInt(this.states[i].name);
            if(this.flowchart.checkState(s)) return true;
          }
        }
        flowchart.index = 0;
        flowchart.level = 1;
        ...
        flowchart.checkState = function(s){
        ...
        //check for force_click_all_correct_nodes
        flowchart.checkFinished = function(){
          if(this.force_click_all_correct_nodes){
            for(var i in mdp.states){
              if(i != 0){
                let s = parseInt(mdp.states[i].name);
                if(this.checkState(s)){
                  mdp.displayFeedback("The flowchart still allows to click some nodes! Please find and click them.", false);
                  return false;
                }
              }
            }
    ...
    async move(s0, a, s1) {
      ...
      if (!this.moved) {
        ...
        if(this.confirmMessage != null && nClick < 3){
          ...
          // ugly workaround for blocking key presses after clicking confirm button
          var e = $.Event('keyup');
          if(a == 'left'){
            e.which = 37;
          } else if(a =='up'){
          ...
          //check for clickable States
          if(!this.flowchart.checkFinished()) return this.arrive(s0, 'repeat');
          this.flowchart.reset(true);
        }
    ...
    autoPreSelect(){
      let s = 0;
      do{
        s = 1+ Math.floor(Math.random() * 12);
      } while(this.pre_selected == s);
    ...
    async preClickState(g, s){
      ...
      if(s == -11){
        if(!this.freeze && this.backgroundRect.customStateResponse() == null){
          if(this.show_pre_select) this.unhighlightAllNodes();
          if(this.show_interactive_flowchart) this.flowchart.reset();
        }
        return;
      }
      ...
      if (this.stateLabels && this.stateDisplay === 'click' && !g.highlighted) {
        ...
        if(this.show_interactive_flowchart){
          this.recordQuery('preselect', 'clickable', this.flowchart.checkState(this.pre_selected));

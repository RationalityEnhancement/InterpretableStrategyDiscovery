jsPsych.plugins['roadtrip'] = (function () {
    ...
    Roadtrip = class Roadtrip {
        constructor(display_element, trial_info) {
            ...
            console.log("trial info", trial_info);
            ...
            // updated this part so now it is compartible with the new jsPsych version
            this.htmlToElements = function createElementFromHTML(container, htmlString, child=false) {
        ...
        get_graph_data() {
            ...
            return {
                edges: _.this.map(this.map.edges._data, function (d, idx) {
                    return [d.from, d.to];
                }),
                nodes: x
            }
        ...
        get_trial_info() {
            return {
                "this.city_info": this.city_info,
                "map": this.get_map_name(),
                "graph": this.get_this.graph_data(),
                "start_city": this.start_city,
                "this.this.end_cities": this.end_cities
            }
        }
        ...
        show_reward_on_city(id, city_name, feedback, log = false) {
            ...
            this.map.nodes.update({id: id, label: city_name + "\n-$" + reward})
        ...
        show_route(route, feedback = true, is_practice = false, callback) {
          // console.log("choosen route", route)
            ...
            // console.log("ids", node_ids);
            // console.log("map nodes", self.map.nodes);
            _.forEach(node_ids, function (id) {
              // console.log("id", id);
                self.map.nodes.update({
                    id: id[0],
                    color: {border: self.selected_node_color},
                    borderWidth: self.node_on_route_width
                })
            });
            ...
            let trial_score = Math.max((500 - route_cost - this.lookup_cost), 0)
            let trial_bonus = trial_score/1000;

            // we need to pass the SCORE to experiment.js so that it can processed together with the previous experiment in order to calculate the final bonux
            let feedbacktext = '';
            ...
            if (is_practice) {
                ...
                // feedbacktext = "Incurred route cost: $" + route_cost + ", Score so: $" + SCORE.toFixed(2);
            }
            // log
            ...
            this.data.score = 0;
            this.data.trial_score = trial_score;
        ...
        enable_report_mode() {
            ...
            this.is_selected = _.object(_.map(self.map.edges.getIds(), function (id) {
                return [id, false]
            }))
        ...
        enable_editing() {
            ...
            this.map.net.on("hoverNode", function (evt) {
                _.each(this.map.net.getConnectedEdges(evt.node), function (e) {
                    this.map.edges.update({id: e, width: 2})
                });
            })
            this.map.net.on("blurNode", function (evt) {
                _.each(this.map.net.getConnectedEdges(evt.node), function (e) {
                    this.map.edges.update({id: e, width: 1})
                });
            })
        }
        ...
        draw_graph() {
            ...
            var options = {};
            this.map.nodes = new vis.DataSet(options);
            ...
            this.map.edges = new vis.DataSet(options);
        ...
        draw_graph_from_json() {
            ...
            this.map.net = new vis.Network(container, this.map, this.vis_options);
            this.map.net.storePositions();

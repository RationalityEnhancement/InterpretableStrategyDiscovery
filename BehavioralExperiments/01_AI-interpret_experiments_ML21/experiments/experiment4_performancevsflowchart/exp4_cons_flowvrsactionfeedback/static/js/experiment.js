...
var BLOCKS, CONDITION,CONDITION_TYPE, CONSTANT_DELAY, DEBUG, DEMO, DEMO_TRIALS, N_TRIAL, PARAMS, SCORE, SHOW_PARTICIPANT, STRUCTURE, TALK, TRIALS, calculateBonus, createStartButton, getTrainTrials,getTestTrials, initializeExperiment, psiturk, saveData, with_feedback, with_info, with_reward,
...
$(window).on('load', function() {
  ...
  return delay(300, function() {
    ...
    TRAIN_TRIALS = loadJson("static/json/train_trials_prs.json");
    TEST_TRIALS = loadJson("static/json/test_trials_prs.json");
...
initializeExperiment = function() {
  ...
  leaf_instructions_2 = new Block({
    ...
    {
      return [
  markdown("<h1>Definitions </h1>\n\nTo advance to the next stage, you will need to understand the following definitions: \n\n- The tree consists of nodes (circles) which are connected with arrows. A node that is fully grey is called an ***unobserved*** node. A node that has a positive or a negative reward on it is called an ***observed*** node. \n\n- The tree has 3 ***levels*** defined by the horizontal alignment of the nodes. \n\n- The nodes of ***higher*** levels are ***higher*** up in the tree. That is, the nodes of Level 3 are above the nodes of Level 2 and the nodes of Level 2 are above the nodes of Level 1. \n\nSee the image below to get a better sense of these notions.\n\n" + img('tree-glossary-noroot', 65)),

  markdown("<h1> Definitions </h1>\n Furthermore: \n\n- Each node lies on a path: a sequence of nodes from level 1 to level 3. \n- Best paths are paths where the sum of rewards add up to the highest value. The reward for unobserved nodes counts as 0.\n- For simplicity, ***nodes on best paths are marked green***. \n\n\n" + img('tree-glossary-path', 50, 'flex')),

  markdown("<h1>Flowchart</h1> Understanding the introduced notions allows you to follow flowcharts that convey simple clicking procedures. Look at the image below to see how a flowchart can look like. As you might notice, it encodes a certain strategy of clicking the nodes. In this experiment your goal will be to act according to one such flowchart!." + img('DT2',30)),

  markdown("<h1>Task procedure</h1> \n\n1. Read the flowchart carefully. \n\n2. Think of a node you would like to click \n\n3. Go through the flowchart and answer questions about that node. \n\n4. Click that node if the flowchart landed you in a 'Click it' decision. Otherwise, think of a different node. \n\n5. Once you are sure that you clicked all the nodes the flowchart allows clicking -- that is it would evaluate to 'Don't click it' for every node -- move the monkey through the tree so as to collect as much reward as possible.")];
    }
  });
    ...
    practice2 = new MouselabBlock({
      ...
      showMoneyMade: true,
      show_div_tutor: true,
      divTutor_config:  {'div': "<img src='static/images/DT2.png' style='height: 120px; width: 280px; left: 100px; top:100px'></img>",
    'checks': [['p.isUnobserved.call(this, s, false)', 'p.onLevel.call(this, s, false, 1)']]},
    ...
    questions: ["What is your task in the game?", "How should you make decisions about where to click?", "When should you start climbing the monkey up the tree?", "How are nodes lying on best paths marked?", "What is an unobserved node?"],
    options: [['Follow a strategy of clicking in the Tree of Cash game that is conveyed by a flowchart.', 'Collect all the possible rewards in the tree.', 'Climb the monkey up the top of the tree.'], ['I can click randomly.', 'Based on the flowchart.', 'Based on the expert demonstrations.', ], ['After all the nodes were clicked.', "When all the nodes that led to CLICK IT decision according to the flowchart were clicked.", 'Based on a careful computation of expected rewards.', ], ['They have a high reward on it.', 'They are green.', 'They are unobserved.'], ['Any node the monkey is currently on.', 'A node which has not been clicked in the current trial.', 'A node that has never been clicked in any of the previous trials.']],
    required: [true, true, true, true, true],
    correct: ['Follow a strategy of clicking in the Tree of Cash game that is conveyed by a flowchart.', 'Based on the flowchart.', "When all the nodes that led to CLICK IT decision according to the flowchart were clicked.", 'They are green.', 'A node which has not been clicked in the current trial.']
  });
  ...
  notree_instructions_2 = new Block({
    type: 'instructions',
      ...
      {
        return [
          markdown("<h1>Goal </h1>\n\n The node inspector is helpful only when you know **where** to click. Try to figure that out and help the monkey to get as much cash as possible. The more cash you collect in the test phase, the larger **bonus** you will get!\n\n"),

          markdown("<h1> Help </h1>\n There are six different paths you can choose from. To help you identify the currently most rewarding path its nodes are highlighted green.\n\n\n" + img('tree-glossary-path', 50, 'flex')),

          markdown("<h1>Feedback </h1>\n\nIn order to make things easier for you, you will receive feedback on how well you are doing. Concretely, once you make the first move with the monkey we will tell you if you moved optimally or not: If you move to a node that will not let you get the highest possible total reward, you will be given a time penalty and a message indicating the optimal course of action. Otherwise, you will see a 'Good job' message."), markdown("<h1> Practice </h1>\n\n In the next two rounds, you will be given a chance to try out the node inspector with the feedback.\n\n *Try it out on the next page!* \n")];
      }
    });

    practice2_condition2 =  new MouselabBlock({
        showMoneyMade: true,
        showRightImage: false,
  ...
  quiz1_condition2 = new Block({
    ...
    type: 'survey-multi-choice',
    questions: ["What is your task in the game?", "How should you make decisions about where to click?", "What is a node?", "Will you receive feedback on your performance?"],
    options: [['Maximize the total reward of the monkey.', 'Collect all the possible rewards in the tree.', 'Climb the monkey up the top of the tree.'],
            ...
            ['Yes, on my clicks.', "Yes, on my first move.", 'No.']],
    required: [true, true, true],
    correct: ['Maximize the total reward of the monkey.', 'By figuring out how to use the node inspector in the most intelligent way.', "Every grey circle in the tree.", "Yes, on my first move."]
  });

  instruct_DT_loop_condition2 = new Block({
    timeline: [notree_instructions_2, practice2_condition2, quiz1_condition2],
    loop_function: function(data) {
      var c, i, len, ref;
      console.log(data);
      ref = data[data.length-1].correct;
  ...
  testing = new MouselabBlock({
    ...
    show_div_tutor: true,
    divTutor_config:  {'div': "<img src='static/images/DT1.png' style='height: 175px; width: 560px; left: -20px; top:30px'></img>",
  ...
  experiment_timeline = (function() {
    if(CONDITION == 0){
      ...
      //instruct_mouselab, instruct_DT_loop, testing_block, inter_block, end_block
      return [instruct_mouselab, instruct_DT_loop, testing_block, inter_block, end_block];
    } else if (CONDITION == 1) {
      console.log(" -- Control timeline");
      return [instruct_mouselab_condition2 , instruct_DT_loop_condition2, testing_block_condition2, inter_block, end_block_condition2];
    } else {

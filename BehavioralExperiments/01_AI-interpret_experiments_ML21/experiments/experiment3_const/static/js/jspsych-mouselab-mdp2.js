  ...
  MouselabMDP = class MouselabMDP {
                    ...
                    if(index == 6){
                      ...
                      for(var i in mdp.states){
                        if(i != mdp.pre_selected){
                          mdp.states[i].deleteClickEvents();
                        }
                      }
                      mdp.backgroundRect.__eventListeners["mousedown"] = [];

                    } else {
                      // don't click
                      mdp.states[mdp.pre_selected].deleteClickEvents();
                    }
                  }
    ...
    resetClickEvents(){
      for(var i in this.states){
        this.states[i].setUpClickEvents();
      }
      if(this.show_pre_select){
        this.backgroundRect.on('mousedown', () => {
          ...
          if(this.show_interactive_flowchart  && !this.freeze && this.flowchart.force_correct_clicking) this.resetClickEvents();
        });
      }
    }
    ...
    async preClickState(g, s){
      ...
      if(this.stateLabels && this.stateDisplay === 'click' && g.highlighted && g.revealed && this.show_interactive_flowchart){
        // -> already clicked
        g.unhighlight();
        this.resetClickEvents();
        this.flowchart.reset();
    ...
    buildMap() {
      ...
      if(this.show_pre_select){
        ...
        this.canvas.add(this.backgroundRect);
      }
  ...
  State = class State {
    ...
    setUpClickEvents(){
      if (!mdp.showParticipant) {
        ...
        if(this.circle.__eventListeners == undefined || this.circle.__eventListeners["mousedown"].length == 0){
        ...
        if(this.circle.__eventListeners["mouseover"] == undefined || this.circle.__eventListeners["mouseover"].length == 0){
          this.circle.on('mouseover', () => {
            return mdp.mouseoverState(this, this.name);
          });
        }

        if(this.circle.__eventListeners["mouseout"] == undefined || this.circle.__eventListeners["mouseout"].length == 0){
    ...
    deleteClickEvents(){
        this.circle.__eventListeners["mousedown"] = [];
        this.circle.__eventListeners["mouseover"] = [];
        this.circle.__eventListeners["mouseout"] = [];
        this.label.__eventListeners["mousedown"] = [];
    }

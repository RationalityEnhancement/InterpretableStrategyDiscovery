...
def print_LC(dfs, key, title='', ylabel = 'PA', xlabel = 'Trial', measure='median'):
    ...
    if measure == 'mean':
        ...
        for df, label, col in zip(dfs, ['Training', 'No-Training'], ["#3977AF", "#EF8536"]):
...
def create_barplot(df1, df2, key, bins):
    ...
    print(pos)
    plt.clf()
    plt.bar( pos, exp_norm, yerr=[exp_err_down, exp_err_up],  width= dif*0.7, align='edge', label="Training", capsize=4, error_kw=dict(lw=1), color="#3977AF")
    plt.bar( pos, cont_norm, yerr=[cont_err_down, cont_err_up], width= dif*-0.7, align='edge', label="No Training", capsize=4, error_kw=dict(lw=1), color= "#EF8536")

...
from decision_tree_imitation_learning import get_program_set
from PLP.dt_utils import extract_plp_from_dt
from hyperparams import NUM_PROGRAMS
...
def check_for_additional_nodes(preds_in_cnfs, set_preds):
    """
    ...
    new_preds_in_cnfs = copy.deepcopy(preds_in_cnfs)
    prev_preds = {}
    for pred in set_preds:
        cnf_count = -1
        for cnf in preds_in_cnfs:
            cnf_count += 1
            sub_count = -1
            for sub_pred in cnf:
                sub_count += 1
                if (pred == sub_pred or pred == sub_pred[4:-1]) and \
                    ...
                    ind = cnf.index(sub_pred)
                    prev_preds[pred] = [cnf[:ind]]
                elif (pred == sub_pred or pred == sub_pred[4:-1]) and \
                    pred in prev_preds.keys():
                    ind = cnf.index(sub_pred)
                    new_name = ' new' * len(prev_preds[pred])
                    if cnf[:ind] not in prev_preds[pred]:
                        if 'not' in sub_pred:
                            new_set_preds.append(sub_pred[4:-1] + new_name)
                        else:
                            new_set_preds.append(sub_pred + new_name)
                        new_preds_in_cnfs[cnf_count][sub_count] += new_name
                        prev_preds[pred].append(cnf[:ind])
                    elif prev_preds[pred].index(cnf[:ind]) != 0:
                        which = prev_preds[pred].index(cnf[:ind])
                        new_name = ' new' * which
                        if 'not' in sub_pred:
                            not_pred = new_preds_in_cnfs[cnf_count][sub_count]
                            change = 'not(' + not_pred[4:-1] + new_name + ')'
                            new_preds_in_cnfs[cnf_count][sub_count] = change
                        else:
                            new_preds_in_cnfs[cnf_count][sub_count] = new_name
                        
    #print(new_preds_in_cnfs)
    #print(new_set_preds)
    return new_preds_in_cnfs, new_set_preds

def dnf2cnf_list(program_string):
    """
    Transform a string encoding a DNF into a list of CNFs.
    
    CNF's are themselves lists of strings for the predicates.
    """
    cnfs = program_string.split(' or ')
    cnfs = [c[2:-2] for c in cnfs] ##remove (( ))
    cnfs = [re.sub('not ', 'not', c) for c in cnfs]
    preds_in_cnfs = [c.split(' and ') for c in cnfs]
    
    for cnf in preds_in_cnfs: ## correcting for spacing and among(A)
        c = -1
        for pred in cnf:
            c += 1
            if 'not (not' in pred:
            ...
            if re.match('among', pred):         
                if not re.search(':', pred) and not re.search('and', pred):
                    cnf[c] = pred[6:-1]          

    for cnf in preds_in_cnfs: ##correcting for among(A and B: C)
        c = -1
        real_preds = []
        to_remove = []
        for almost_pred in cnf:
            c += 1
            open_ = sum([el == '(' for el in almost_pred])
            close_ = sum([el == ')' for el in almost_pred])
            if open_ > close_:
                real_pred = almost_pred + ' and ' + cnf[c+1]
        ...
        num_removed = 0
        for i in to_remove:
            cnf.remove(cnf[i-num_removed])
            num_removed += 1
    return preds_in_cnfs

def get_used_preds(preds_in_cnfs):
    """
...
def add_non_leaf_positive_pred_to_digraph(pred, cnf, c, new_string, set_preds, \
                                                        ordering, ids, saved):
    """
    ...
    if ordering[pred] == 0:
        ...
        new_string += ids[pred] + ' -> ' + ids[cnf[c+1]] + params
        ordering[pred] = 1
...
def add_positive_pred_to_digraph(pred, cnf, c, num_preds, new_string, \
...
def add_non_leaf_negative_pred_to_digraph(pred, cnf, c, new_string, set_preds, \
                                                        ordering, ids, saved):
    """
...
def add_negative_pred_to_digraph(pred, cnf, c, num_preds, new_string, \
                                    set_preds, ordering, ids, saved):
    """
...
def add_cnf_to_digraph(cnf, new_string, set_preds, ordering, ids, saved):
    """
...
def pretty_print(preds_in_cnfs):
    """
...
def dnf2tree(program_string, expr=True):
    ...
    """
    preds_in_cnfs = dnf2cnf_list(program_string)
    ...
    saved = -1
    for cnf in preds_in_cnfs:
        new_string, ordering, saved = add_cnf_to_digraph(cnf, 
                                                         new_string, 
                                                         set_preds, 
                                                         ordering, 
                                                         ids, 
    ...
    pretty_program = pretty_print(preds_in_cnfs)
...
if __name__ == "__main__":
    ...
    with open(folder_path + file_name, 'rb') as handle:
        LPP_object = pickle.load(handle)
    program = LPP_object['program']

    ## expr=False generates a tree with raw predicates in the nodes
    new_string, pretty_program = dnf2tree(program, expr=False)
    
    print("Composing a txt file with the formula...")

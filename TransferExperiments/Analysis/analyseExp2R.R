...
df_complete <- read.csv('data/Experiment2/dataframe_complete.csv')

t <- read.csv('data/Experiment2/dataframe_mortgage_final.csv', sep=";")
t <- t[order(t$pid),]
...
# Filter
df_valid_all <- subset(df_complete  , (status != 6 & status != 2))
df_valid_all$PB_combined <- (as.numeric(df_valid_all$PB3) + as.numeric(df_valid_all$PB6) + (1-df_valid_all$mortgageChoice))
df_valid_all$FS_combined_bin <- (as.numeric(df_valid_all$PB3) + as.numeric(df_valid_all$PB6) + (1-df_valid_all$mortgageChoice)) == 0
df_valid_all$idf_mean <- rowMeans(data.frame(df_valid_all$idf6_12,df_valid_all$idf6_9,df_valid_all$idf0_6,df_valid_all$idf0_3))

df_valid <- subset(df_valid_all , sanityFail == 0 & mortgageChoice != -1)
...
par(mar=c(5,6,4,1)-1)
p  <- ggplot(aes(y = value, x = task, fill=conditionType), data = pd)  + geom_bar(stat="identity", position=position_dodge()) 
p <- p + scale_fill_manual(values=c("#FF7F0E", "#1F77B4"))
p <- p + xlab("Task") + ylab("Group proportion") + ggtitle('Far-sighted beahvior per task and group')  + labs(fill = "Group")
p <- p  + scale_y_continuous(labels = percent, minor_breaks = seq(0 , 0, 0)) + theme(text = element_text(size = 35), plot.title = element_text(size=35))  + theme(plot.title = element_text(hjust = 0.5)) + scale_x_discrete(guide = guide_axis(n.dodge=2))
p <- p +   geom_errorbar(aes(ymin=value-err, ymax=value+err), width=.1, position=position_dodge(.9)) +  theme(legend.position="bottom")
p + theme(plot.margin=unit(c(0,1,0,0),"cm"))
...
p <- ggplot(aes(y = idf, x = idf_type, fill = Group), data = pd2) + geom_boxplot() 
p <- p + xlab("Parameter") + ylab("Discount Factor") + ggtitle('Discount factors per group') 
p <- p +  theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(values = c("#EF8536", "#3977AF",  "#3977AF")) 
p <- p + theme(text = element_text(size = 35)) + theme(legend.position = "right") + scale_x_discrete(labels = c('t=0/k=3', 't=0/k=6', 't=6/k=3', 't=6/k=6', 'Mean'))
p <- p + theme(legend.position="bottom",  axis.title.x= element_text(vjust = -0.15)) 

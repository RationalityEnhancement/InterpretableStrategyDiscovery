...
class ImmediatePolicy(Policy):
    ...
    def act(self, state):
        immediate = set(self.env.tree[0])
        available = set([act for act in self.env.actions(state)])
        to_click = immediate.intersection(available)
        if to_click == set([]):
            return self.env.term_action
        return random.sample(to_click, 1)[0]

    def name(self):
        return "immediate_states" 
...
class FinalPolicy(Policy):
    ...
    def act(self, state):
        final = set([n[0] for n in self.env.subtree if len(n) == 1])
        available = set([act for act in self.env.actions(state)])
        to_click = final.intersection(available)
        if to_click == set([]):
            return self.env.term_action
        return random.sample(to_click, 1)[0]
...
class DFSPolicy(Policy):
    def __init__(self, alpha=20, omega=-4):
        super().__init__()
        self.alpha = alpha
        self.omega = omega

    def act(self, state):
        available = [act for act in self.env.actions(state)]
        if self.env.expected_term_reward(state) >= self.alpha:
            return self.env.term_action
        wanted = [self.env.node_quality(x, state).expectation() > self.omega for x in available]
        r_av = []
        for i in range(len(available)):
            if wanted[i]: r_av.append(available[i])
        if r_av == []:
            return self.env.term_action
        return r_av[0]
...
class BRFSPolicy(Policy):
    ...
    def act(self, state):
        ...
        if self.env.expected_term_reward(state) >= self.alpha:
            return self.env.term_action
        wanted = [self.env.node_quality(x, state).expectation() > self.omega for x in available]
        available = [available[i] for i in range(len(available)) if wanted[i] is True]
        r_av = []
        for i in range(len(available)):
            if wanted[i]: r_av.append(available[i])
        order = [node for level in self.env.tree for node in level if level != []]
        for node in order:
            if node in r_av: return node
        return self.env.term_action
...
class BEFSPolicy(Policy):
    ...
    def act(self, state):
        available = [act for act in self.env.actions(state)]
        not_available = [act for act in range(len(self.env.tree)) if act not in available]
        candidates = [child for clicked in not_available for child in self.env.tree[clicked] if child not in not_available]
        wanted = [self.env.node_quality(x).expectation() > self.omega for x in candidates]
        r_ca = []
        for i in range(len(candidates)):
            if wanted[i]: r_ca.append(candidates[i])
        candidates = r_ca
        if candidates == [] or self.env.expected_term_reward(state) >= self.alpha:
            return self.env.term_action
        return max(candidates, key=lambda x: self.env.node_quality(x).expectation())

    def name(self):
        return "best_first_search" 


## TODO: learn to compute BMPS policy and to generate BMPS clicks

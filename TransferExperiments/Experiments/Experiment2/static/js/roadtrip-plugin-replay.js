...
jsPsych.plugins['roadtrip-plugin'] = (function(){
  ...
  Roadtrip = class Roadtrip{
    constructor(display_element, trial_info) {
      ...
      this.graph_data2 = {"nodes": trial_info['nodes']};
      this.start_city = this.id2name(trial_info['start_nodes'])[0];
      this.end_cities = this.id2name(trial_info['end_nodes']);
      this.clicks = trial_info['clicks'];

      //this.city_names = Object.keys(this.city_info);
      this.is_practice = false;

      let edges = [];
      for(let n in trial_info['nodes']){
        n = trial_info['nodes'][n];
        for(let edge of n["outEdge"]){
          edges.push([n["id"], edge]);
        }
      }
      this.graph_data = {'nodes': trial_info['nodes'], 'edges': edges};
      ...
      $('#feedbackroadtrip p').html('pid: ' + String(trial_info['PID']) + ' -- Trial: ' + String(trial_info['trial']) + '<br>Condition: ' + String(trial_info['condition']) + '<br>PA: ' + String(trial_info['pa']));
    ...
    async start_simulation(){
      ...
      for(let click of this.clicks){
        await sleep(1000);
        this.map.nodes.update({id : click, label: "\n" + this.id2name(click)[0] + "\n$" + this.graph_data2['nodes'][String(click)].reward, color: 'red'});
      }
    };
    ...
    show_feedback(feedbacktext,duration,callback){
    ...
    name2id(city_names){
      ...
      for(let c of city_names){
        for(let n in this.graph_data2.nodes){
          if(this.graph_data2.nodes[n].label == c){
            res.push(this.graph_data2.nodes[n].id);
            break;
          }
        }
      }
    ...
    id2name(ids){
      ...
      for(let id of ids){
        for(let n in this.graph_data2.nodes){
          if(this.graph_data2.nodes[n].id == id){
            res.push(this.graph_data2.nodes[n].label);
            break;
          }
        }
      }
      ...
      for(var i=1;i<node_ids.length && i<route.length;i++){
        for(var j=0;j<node_ids[i].length;j++){
          if(this.use_end_city_reward || !this.end_cities.includes(route[i])){
            //expected score
            if(this.is_reward_revelaed(node_ids[i][j])) route_cost_expected += this.get_reward(this.map.nodes.get(node_ids[i][j]).label);
    ...
    enable_report_mode(){
      ...
      this.map.net.on("click",function(e){
        for(var i=0;i<e.edges.length;i++){
          ...
          //log_data({"event_type": "Select edge", "event_info" : {"from" : city_from, "to" : city_to, "mode" : (this.is_selected[e.edges[i]] ? "off" : "on")}})

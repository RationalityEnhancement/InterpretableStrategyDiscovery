from PLP.policy import StateActionProgram
...
def extract_plp_from_dt(estimator, features, feature_log_probs):
    ...
    ## only the majority nodes, where True option prevails, are chosen to create the program
    ...
    if not isinstance(disjunctive_program, StateActionProgram):
        disjunctive_program = StateActionProgram(disjunctive_program, tree=estimator)

    disjunctive_program.set_tree(estimator)
